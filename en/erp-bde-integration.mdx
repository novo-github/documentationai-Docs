...
description: >-
  Import production data into Novo AI from ERP, BDE/MES, or Excel/CSV —
  connection methods, data requirements, validation rules, and the customer
  handover package.
---
## Introduction
...
| --------------------------------------------------------------------- | -------------------------- | ------------------------------------- | -------------------------------------------------- | --------------------------------------- |
| [**Direct ERP planning import**](#direct-erp-integration)             | ERP -> Novo AI             | One planned operation per record      | Planning/order fields and stable resource mapping  | Read-only source access                 |
| [**Existing BDE/MES import**](#erp--existing-bde-mes-integration)     | BDE/MES -> Novo AI         | One recorded activity/state interval  | Interval, machine, state, and order identifiers    | Read-only source typically sufficient   |
| [**Temporary Excel/CSV import**](#temporary-erp-import-via-excel-csv) | File -> Novo AI            | One planned operation per row         | Upload template with agreed formats and units      | No live system connection required      |
| [**Machine & production export**](en/erp-data-export)                 | Novo AI -> customer system | One continuous machine-state interval | Writable target and accepted state/duration fields | Read/write or equivalent API operations |
...
<Tabs>
  ...
  <Tab title="MQTT">
    For shop-floor or BDE/MES systems that already publish over MQTT, Novo AI can subscribe to the relevant topics directly.

    - **Best for:** near-real-time streaming of machine-state or BDE intervals
    - **Access:** MQTT broker credentials, scoped to read-only subscription on the agreed topics
    - **Update model:** event-driven / streaming
  </Tab>

  <Tab title="Direct database connection">
    Novo AI connects directly to your ERP or BDE database using a dedicated read-only account.

    - **Best for:** systems without a convenient API layer, where planning data already lives in accessible views or tables
    - **Access:** read-only database user, scoped to a single schema, view, or table
    - **Update model:** scheduled polling (for example, every few minutes)
  </Tab>

  <Tab title="Single read-only table or view">
    The simplest option: expose one table or view containing just the fields Novo AI needs, and grant a read-only account access to that object only.

    - **Best for:** minimizing exposure. No access to the rest of the ERP schema
    - **Access:** read-only user with `GRANT SELECT` on one table/view
    - **Update model:** scheduled polling
  </Tab>
</Tabs>

<Callout kind="info" collapsed="false">
  Exact technology, endpoints, and credentials are still agreed per project — see [Access, connectivity, and implementation inputs](#access-connectivity-and-implementation-inputs) below. This is a menu of supported options, not a requirement to change your existing systems.
</Callout>

### Example: read-only integration table

If you choose the direct-database or single-table/view option, the fastest path is to expose the planning fields already defined in [Required ERP -> Novo AI planning fields](#required-erp---novo-ai-planning-fields) as one dedicated table or view.
...
<Callout kind="tip" collapsed="false">
  This table is a starting point, not a fixed schema. Optional fields (see [Optional planning fields](#optional-planning-fields)) can be added as extra nullable columns, and the same approach — one export table, one read-only account — also works for exposing [existing BDE/MES intervals](#minimum-interval-data).
</Callout>
...
<Callout kind="info" collapsed="false">
  **Data Export:** Novo AI also exports machine-state and BDE production data intervals to customer receiving systems. For export fields, interval rules, and write-back design, see [Data Export](en/erp-data-export).
</Callout>
...
The data dictionary defines **what** information is exchanged. The following customer-side inputs are also needed to implement the connection reliably. Exact technology choices depend on the customer environment and must be agreed during the project — see [Connection methods](#connection-methods) above for the supported transport options.
